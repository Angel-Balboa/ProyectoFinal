...
class Eh_PayPal_Express_Payment extends WC_Payment_Gateway {
    ...
    public function payment_scripts() {
        if (is_checkout()) {
            ...
            wp_register_script('eh-express-style', EH_PAYPAL_MAIN_URL . 'assets/js/eh-express-script.js');
            wp_enqueue_script('eh-express-style');
            wp_localize_script('eh-express-style', 'page_name', 'checkout');
        }
    }
        ...
        switch ($action_code) {
            ...
            case 'express_details':
                ...
                try {
                    ...
                    //for guest users billing address is updated if payment processed from cart and product page
                    if(!is_user_logged_in()){ 

                        $page_referer = $_GET['p'];
                        $parts = parse_url($page_referer);
            ...
            case 'finish_express':
                ...
                try {
                    ...
                    if ($response['ACK'] == 'Success' || $response['ACK'] == 'SuccessWithWarning') {
                        ...
                        $user_creation = $this->create_account($checkout_post,$order_id);
    ...
    public function fill_checkout_fields_on_review() {
        $checkout_shipping = WC()->session->eh_pe_checkout['shipping'];
        if (isset(WC()->session->eh_pe_checkout) || !empty($checkout_shipping)) {
            foreach (WC()->session->eh_pe_checkout['shipping'] as $key => $value) {
                if ($value) {
    ...
    */
    ...
    public function create_account($checkout_post,$order_id) {

        if (isset($checkout_post['createaccount']) && $checkout_post['createaccount']) {
            ...
            if (!empty($checkout_post['billing_email'])) {
                ...
                if (!is_wp_error($userID)) {
                    ...
                    $user_password = (isset($checkout_post['account_password']) ? esc_attr($checkout_post['account_password']) :'');
